...
updated to only show result for selected targets - Faad Ghoraishi
version_added: "1.0.0"
requirements: [ AIX ]

A taget name could be of the following form all in quotes:
...
def build_nim_node(module):
    ...
    # faad1
    # ========================================================================
    # Updae by Faad Ghoraishi to only show target list
    # ========================================================================
    
    target_list = expand_targets(NIM_PARAMS['targets'])
    logging.debug('NIM - Target list: {}'.format(target_list))
    keys = set(target_list).intersection(standalones)
    logging.debug('key Target list: {}'.format(keys))
    ###keys = set(target_list).intersection(set(standalones.keys()))
    results = {k:standalones[k] for k in keys}

    NIM_NODE['standalone'] = results
